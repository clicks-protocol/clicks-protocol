...
class ClicksClient {
    ...
    constructor(signerOrProvider, options) {
        ...
        this.referral = new ethers_1.Contract(this.addresses.referral, abis_1.REFERRAL_ABI, signerOrProvider);
     ...
     */
    async getReferralNonce(agentAddress) {
        return this.referral.referralNonces(agentAddress);
    }
    /**
     ...
     */
    async buildReferralApprovalTypedData(agentAddress, referrerAddress, deadline) {
     ...
     */
    async signReferralApproval(agentAddress, referrerAddress, deadline) {
     ...
     */
    async registerReferralWithSig(agentAddress, referrerAddress, deadline, signature) {
        const tx = await this.referral.registerReferralWithSig(agentAddress, referrerAddress, deadline, signature);
        return { tx };
    }
    /**
     ...
     */
    async quickStartWithReferral(amount, agentAddress, referrerAddress, deadline, signature, options) {
        const treasury = await this.quickStart(amount, agentAddress, undefined, options);
        try {
            const referral = await this.registerReferralWithSig(agentAddress, referrerAddress, deadline, signature);
    ...
    requireSigner() {
        const candidate = this.signerOrProvider;
