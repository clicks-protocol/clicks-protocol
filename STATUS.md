...
**Settlement Safety Stack, Repository-Stage:** V1 bleibt kompatibel. Receipt V2 ergaenzt Idempotency-Key, Business-Event, Authorization Reference, Request-/Quote-Hash, Settlement Reference, Witness States, Delivery Evidence, Retry Policy und Reconciliation History. Die State-Machine erlaubt nur definierte Uebergaenge. Retry ist fail-closed und ausschliesslich bei `failed_before_transfer` innerhalb der expliziten Attempt-Grenze moeglich; `unknown_settled` kann niemals automatisch erneut eingereicht werden. Die read-only Reconciliation Engine prueft Chain- und optionale externe Witness-Evidence, ohne eine Zahlung ausloesen zu koennen. Ein append-only Ledger verkettet Receipt-Versionen kryptografisch und blockiert Idempotency-Konflikte. Fuenf MCP Read-Tools verifizieren Receipts, Status, Policy und Receipt Trails. Direkte, ACP- und x402-Ingress-Adapter erzeugen nur Metadaten. Der x402 Worker ist ein Repository-Beispiel fuer bereits upstream verifizierte Events, keine live Payment-Integration. 22 SDK-Tests, 232 Hardhat-Tests, MCP-Build und ACP-Bundle-Check sind gruen. Noch nicht auf npm veroeffentlicht und kein Contract-Deploy.

**Security und Release-Vorbereitung:** ACP auf Virtuals ACP Node `0.1.8`, Account Kit `4.88.4`, ethers `6.17.0` und viem `2.55.5` aktualisiert. Gezielt gepinnte transitive Sicherheitsupdates beseitigen alle zuvor gemeldeten High- und Critical-Findings. `npm audit`: ACP 0 high/0 critical, SDK 0 Findings, MCP 0 Findings. GitHub `main` enthaelt den vollstaendigen Safety Stack und die Release-Kandidaten SDK `0.3.0` sowie MCP `0.4.0`. npm-Publish ist blockiert, weil der lokal konfigurierte npm-Token HTTP 401 liefert. Registry-Manifest bleibt deshalb korrekt auf den live veroeffentlichten Versionen Registry `1.0.3` und MCP `0.3.2`. ACP wurde mit dem neuen Code noch nicht neu gestartet.

**ACP Service:** `com.clicks.acp-service` laeuft wieder kontrolliert als genau eine Instanz. `run.sh` nutzt den absoluten Node-22-Pfad und erweitert `PATH`, damit `npx` und dessen Node-Interpreter unter launchd verfuegbar sind. Der Start unterdrueckt die Ausfuehrung bereits vorhandener funded Jobs waehrend der Hydrierung. Verifizierter Startcheck: `0 active job(s): none`. Repository-stage ist der Service jetzt an Receipt V2 und das lokale append-only Ledger angebunden: Job-ID als Idempotency-Key, geplantes Receipt vor Ausfuehrung, `unknown_settled` bei mehrdeutigem Fehler und keine Wiederholung bereits erfasster Jobs. Der laufende Service wurde fuer diese neue Code-Stufe nicht neu gestartet. Es wurde kein Auftrag simuliert und keine Onchain-Transaktion ausgeloest.
