...
class ClicksSDKBridge:
    ...
    def _validate_address(addr: str) -> str:
        """Validate Ethereum address format to prevent injection."""
        import re
    ...
    def _call_sdk(self, method: str, **kwargs) -> dict:
        ...
        Uses environment variables for sensitive data (private key, RPC URL)
        and JSON-serialized params to prevent command injection.
        """
        # Validate any address parameters
        for key, val in kwargs.items():
            if 'address' in key.lower() and isinstance(val, str):
                self._validate_address(val)

        # Whitelist allowed methods to prevent arbitrary code execution
        allowed_methods = {
            'quickStart', 'withdrawYield', 'simulateSplit',
            'getAgentInfo', 'getAgentYieldBalance', 'getUSDCBalance',
            'getYieldInfo', 'registerAgent', 'receivePayment',
            'setOperatorYieldPct', 'approveUSDC',
        }
        if method not in allowed_methods:
            return {"success": False, "error": f"Unknown SDK method: {method}"}

        # Pass params via environment, not string interpolation
        ...
        (async () => {
            const provider = new ethers.JsonRpcProvider(process.env._CLICKS_RPC);
            const wallet = new ethers.Wallet(process.env._CLICKS_KEY, provider);
            const sdk = new ClicksClient(wallet);
            const params = JSON.parse(process.env._CLICKS_PARAMS);
            const args = Object.values(params);

            try {
                const result = await sdk[process.env._CLICKS_METHOD](...args);
        ...
        env = {
            **dict(os.environ),
            '_CLICKS_RPC': self.rpc_url,
            '_CLICKS_KEY': self.private_key,
            '_CLICKS_METHOD': method,
            '_CLICKS_PARAMS': params_json,
        }
        try:
    ...
    def get_agent_info(self, agent_address: str) -> dict:
        ...
        (async () => {
            ...
            const wallet = new ethers.Wallet(process.env._CLICKS_KEY, provider);
            const sdk = new ClicksClient(wallet);
            const addr = process.env._CLICKS_ADDR;
            try {
                const [info, yieldBal, usdcBal] = await Promise.all([
                    sdk.getAgentInfo(addr),
                    sdk.getAgentYieldBalance(addr),
                    sdk.getUSDCBalance(addr),
                ]);
        ...
        env = {
            **dict(os.environ),
            '_CLICKS_RPC': self.rpc_url,
            '_CLICKS_KEY': self.private_key,
            '_CLICKS_ADDR': agent_address,
        }
        try:
            result = subprocess.run(
                ["node", "-e", script],
                capture_output=True, text=True, timeout=60, cwd=self.sdk_path, env=env,
            )
            if result.returncode != 0:
                return {"success": False, "error": result.stderr.strip()}
            return json.loads(result.stdout.strip())
